...
def document_table(p,lang='tr'):
    ...
    label=('Directives' if en else 'Yönergeler') if p['slug']=='about-3-2' else ('Accreditation process documents' if en else 'Akreditasyon süreci belgeleri')
    if p['slug']=='about-3-4':label='Guides and handbooks' if en else 'Kılavuz ve elkitapları'
    if p['slug']=='about-3-3':label='Other documents' if en else 'Diğer belgeler'
...
def content_for(p):
    s=p['slug']
    if s in ('akreditasyon-süreci','about-3-2','about-3-4','about-3-3'):return document_table(p)
...
def inner(p):
    ...
    if p['slug'] in ('akreditasyon-süreci','about-3-2','about-3-4','about-3-3'):page_class='wrap inner-page documents-page'
